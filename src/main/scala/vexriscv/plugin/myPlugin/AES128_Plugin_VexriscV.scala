...
case class AES128_Plugin_VexriscV(encoding : MaskedLiteral = M"-----------------000-----0001011") extends Plugin[VexRiscv]{

  object IS_AES_VexriscV extends Stageable(Bool)
  object CALC_VexriscV extends Stageable(Bits(32 bits))
  ...
  override def setup(pipeline: VexRiscv): Unit = {
    ...
    decoderService.addDefault(IS_AES_VexriscV, False)
    decoderService.add(
      key = encoding,
      List(
        IS_AES_VexriscV              -> True,
  ...
  override def build(pipeline: VexRiscv): Unit = {
    ...
    memory plug new Area{
      ...
      insert(CALC_VexriscV) := xored
    }

    writeBack plug new Area {
      import writeBack._

      when(input(IS_AES_VexriscV)) {
        output(REGFILE_WRITE_DATA) := input(CALC_VexriscV)
      }
    }
  }
